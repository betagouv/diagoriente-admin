...
import lignediag3 from '../assets/pdf/lignediag3.png';
import logov3Diag from '../assets/pdf/Logo_Diagoriente.png';
...
export function pdf2(parcours: any, get: any, user: any, div: any = false) {
  console.log('parametre 1', parcours);
  console.log('parametre 2', get);
  console.log('parametre 3', user);
  ...
  const competences = get.globalCopmetences;
  ...
  const pdfTitle = `Carte de compétences - ${firstName[0].toUpperCase()}${firstName.slice(1)} ${lastName[0].toUpperCase()}${lastName.slice(1)}.pdf`;
...
export function pdf(parcours: any, getParcours: any, authUser: any, div: any = false) {
  ...
  doc.setTextColor(100, 100, 100);
  const interests = getParcours.data.globalInterest.map((el: any) => el.title.split('/').map((el: string) => el.trim()));
